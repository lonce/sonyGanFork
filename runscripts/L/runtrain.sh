...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        -n|--name) name="$2"; shift ;;
		-h|--help) echo "usage:  runtrain.sh -n outname configfile"; exit 1;;
        *) configfile=$1 ;;
    esac
    shift
done
...
python train.py  ${namearg} -c $configfile -s 5000 -l 5000 > $logdir/stdout.txt 2>$logdir/stderr.txt &
echo  "train.py --restart  ${namearg} -c $configfile -s 5000 -l 2000 > $logdir/stdout.txt 2>$logdir/stderr.txt"
